...
void Manual::display() const{
  ...
  std::cout << getTitle() << " is written by " << getAuthor() << " company for device: " << getDevice() << ". " << "Website: " << website_ << ". Page Count: " << getPageCount() << ". " << digital << " available digitally. \n";
}

void Manual::displayFilter(const std::string& key)
  {
  std::string delimiter = "-";
  int pos = getDevice().find(delimiter);
  std::string device = getDevice().substr(0, pos);
  if (device_ == key){
    display();
  }
}